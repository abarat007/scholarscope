...
export default function SearchResults({ query }: { query: string }) {
  ...
  const run = useCallback(
    async (nextMode: SearchMode, nextRerank: boolean, nextOptimize: boolean) => {
      setLoading(true);
      setError(null);
      try {
        const res = await search(query, nextMode, {
          k: 10,
          rerank: nextRerank,
    ...
    <div>
      ...
      <div className="mb-8 flex flex-wrap items-end justify-between gap-4 border-b border-line pb-4">
        <div className="flex flex-wrap gap-6">
            ...
            <label className="flex cursor-pointer items-center gap-2 font-mono text-xs uppercase tracking-widest text-secondary">
              <input
                ...
                onChange={(e) => {
                  ...
                  run("hybrid", e.target.checked, optimize);
                }}
          ...
          <label
            className="flex cursor-pointer items-center gap-2 font-mono text-xs uppercase tracking-widest text-secondary"
            title="Rewrite the query before searching: keyword expansion for BM25, a hypothetical answer passage (HyDE) for dense vector. Costs one extra LLM call."
          >
            <input
              type="checkbox"
              checked={optimize}
              onChange={(e) => {
                ...
                run(mode, rerank, e.target.checked);
              }}
              className="h-3.5 w-3.5 appearance-none border-2 border-foreground bg-background checked:bg-foreground focus-visible:outline focus-visible:outline-2 focus-visible:outline-foreground focus-visible:outline-offset-2"
            />
            Optimize query
